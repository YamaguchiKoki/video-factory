...
import { SCHEDULE_UTC_HOUR, S3_PREFIX } from '../lib/constants';
...
const VPC_CONTEXT_KEY =
  `vpc-provider:account=${TEST_ACCOUNT}:filter.isDefault=true:region=ap-northeast-1:returnAsymmetricSubnets=true`;
...
const { template, ecrTemplate } = (() => {
  ...
  return {
    ...
    ecrTemplate: Template.fromStack(app.node.findChild('EcrStack') as cdk.Stack),
  };
})();
