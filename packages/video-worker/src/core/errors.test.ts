...
describe("Domain Error Types", () => {
  ...
  describe("RenderError", () => {
    ...
    it("should be created using helper function", () => {
      const error = createRenderError("RENDER_TIMEOUT", "Timeout", null, {
  ...
  describe("Error Context", () => {
    ...
    it("should allow complex nested context", () => {
      const error = createRenderError("RENDER_FAILED", "Render error", null, {
        composition: { id: "main", width: 1920, height: 1080 },
        frameInfo: { current: 100, total: 1000 },
      });

      expect(error.context.composition).toEqual({
        id: "main",
        width: 1920,
        height: 1080,
      });
