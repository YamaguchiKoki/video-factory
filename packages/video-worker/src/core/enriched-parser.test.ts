...
describe("parseEnrichedScript — JSON パースエラー", () => {
  it("不正な JSON 文字列で JSON_PARSE_ERROR を返す", () => {
    const invalidJson = "{ invalid json }";
    const result = parseEnrichedScript(invalidJson, WAV_PATH);
  ...
  it("空文字列で JSON_PARSE_ERROR を返す", () => {
    const result = parseEnrichedScript("", WAV_PATH);
    expect(result.isErr()).toBe(true);
    if (result.isErr()) {
      expect(result.error.type).toBe("JSON_PARSE_ERROR");
    }
  ...
  it("JSON_PARSE_ERROR は cause に元の Error を保持する", () => {
    const result = parseEnrichedScript("not-json", WAV_PATH);
    expect(result.isErr()).toBe(true);
    if (result.isErr()) {
      expect(result.error.type).toBe("JSON_PARSE_ERROR");
...
describe("parseEnrichedScript — スキーマバリデーションエラー", () => {
  ...
  it("sections が欠損しているとき SCHEMA_VALIDATION_ERROR を返す", () => {
  ...
  it("newsItems が欠損しているとき SCHEMA_VALIDATION_ERROR を返す", () => {
  ...
  it("discussion の phase が無効なとき SCHEMA_VALIDATION_ERROR を返す", () => {
    const invalidPhase = {
  ...
  it("ライン内の speaker が A/B 以外のとき SCHEMA_VALIDATION_ERROR を返す", () => {
      ...
      sections: [
        {
          type: "intro",
          greeting: [
            { speaker: "X", text: "hi", voicevoxSpeakerId: 0, offsetSec: 0, durationSec: 3.0 },
          ],
          newsOverview: [],
        },
...
describe("parseEnrichedScript — 異常系: 空の配列", () => {
  it("discussion block の lines が空のとき SCHEMA_VALIDATION_ERROR を返す", () => {
    ...
    const result = parseEnrichedScript(JSON.stringify(emptyDiscLines), WAV_PATH);
    expect(result.isErr()).toBe(true);
    if (result.isErr()) {
      expect(result.error.type).toBe("SCHEMA_VALIDATION_ERROR");
    }
  ...
  it("intro の greeting も newsOverview も空のとき SCHEMA_VALIDATION_ERROR を返す", () => {
    const emptyIntro = {
      ...minimalEnrichedScript,
      sections: [
    ...
    const result = parseEnrichedScript(JSON.stringify(emptyIntro), WAV_PATH);
    expect(result.isErr()).toBe(true);
    if (result.isErr()) {
      expect(result.error.type).toBe("SCHEMA_VALIDATION_ERROR");
    }
  ...
  it("outro の recap も closing も空のとき SCHEMA_VALIDATION_ERROR を返す", () => {
    const emptyOutro = {
      ...minimalEnrichedScript,
      sections: [
    ...
    const result = parseEnrichedScript(JSON.stringify(emptyOutro), WAV_PATH);
    expect(result.isErr()).toBe(true);
    if (result.isErr()) {
      expect(result.error.type).toBe("SCHEMA_VALIDATION_ERROR");
    }
...
describe("parseEnrichedScript — 正常系: スカラーフィールド", () => {
  it("title が VideoProps に正確に引き継がれる", () => {
    const result = parseEnrichedScript(MINIMAL_JSON, WAV_PATH);
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      expect(result.value.title).toBe("テストタイトル");
    }
  });
  ...
  it("totalDurationSec が VideoProps に正確に引き継がれる", () => {
    ...
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      expect(result.value.totalDurationSec).toBe(30.0);
    }
  });
});
  ...
  it("複数の newsItems が全て引き継がれる", () => {
    const script = {
      ...
      newsItems: [
        ...
        { id: "news-3", title: "ニュース3", sourceUrl: "https://example.com/3" },
      ],
    };
    const result = parseEnrichedScript(JSON.stringify(script), WAV_PATH);
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      expect(result.value.newsItems).toHaveLength(3);
      expect(result.value.newsItems.map((n) => n.id)).toEqual(["news-1", "news-2", "news-3"]);
    }
  });
});
...
describe("parseEnrichedScript — 正常系: ライン平坦化", () => {
  it("全セクションのライン数の合計が VideoProps.lines の長さになる", () => {
    ...
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      expect(result.value.lines).toHaveLength(7);
    }
  });
  ...
  it("各ラインの audioPath が渡した wavPath と一致する", () => {
    ...
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      result.value.lines.forEach((line) => {
  ...
  it("各ラインの startSec が入力 offsetSec と一致する", () => {
    ...
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      const actualOffsets = result.value.lines.map((l) => l.startSec);
  ...
  it("各ラインの durationSec が入力 durationSec と一致する", () => {
    ...
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      const actualDurations = result.value.lines.map((l) => l.durationSec);
  ...
  it("ラインは sections の出現順（intro → discussion → outro）に並ぶ", () => {
    ...
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      const offsets = result.value.lines.map((l) => l.startSec);
      const isMonotonicallyIncreasing = offsets.slice(1).every((offset, i) => offset > offsets[i]);
...
describe("parseEnrichedScript — 正常系: SectionMarkers", () => {
  it("SectionMarkers の総数が正しい（1 intro + 3 discussion blocks + 1 outro = 5）", () => {
    ...
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      expect(result.value.sectionMarkers).toHaveLength(5);
    }
  });
  ...
  describe("IntroSectionMarker", () => {
    it("type が 'intro' である", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const intro = result.value.sectionMarkers.find((m) => m.type === "intro");
  ...
  describe("DiscussionSectionMarker（各 block が独立した marker になる）", () => {
    it("discussion の各 block に対して DiscussionSectionMarker が生成される", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const discussions = result.value.sectionMarkers.filter((m) => m.type === "discussion");
    ...
    it("summary block の startSec が 7.0、endSec が 12.0 である", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const summary = result.value.sectionMarkers.find(
          (m) => m.type === "discussion" && m.phase === "summary",
        );
    ...
    it("background block の startSec が 12.0、endSec が 17.0 である", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const background = result.value.sectionMarkers.find(
    ...
    it("deepDive block の startSec が 17.0、endSec が 22.0 である", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const deepDive = result.value.sectionMarkers.find(
  ...
  describe("OutroSectionMarker", () => {
    it("type が 'outro' である", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const outro = result.value.sectionMarkers.find((m) => m.type === "outro");
  ...
  it("全 SectionMarker の startSec は単調非減少の順で並ぶ", () => {
    ...
    expect(result.isOk()).toBe(true);
    if (result.isOk()) {
      const starts = result.value.sectionMarkers.map((m) => m.startSec);
      const isMonotonicallyNonDecreasing = starts.slice(1).every((start, i) => start >= starts[i]);
...
describe("parseEnrichedScript — プロパティベーステスト（不変条件）", () => {
  const validEnrichedScriptArb = fc
    ...
    .map(({ introLineCount, discLineCount, outroLineCount }) => {
      ...
      const totalDuration = (introLineCount + discLineCount + outroLineCount) * dur;

      return {
        title: "プロパティテスト",
        totalDurationSec: totalDuration,
        outputWavS3Key: "output/test.wav",
        newsItems: [{ id: "n1", title: "News", sourceUrl: "https://x.com" }],
        sections: [
          {
            type: "intro",
          ...
          },
          {
            type: "discussion",
            newsId: "n1",
            blocks: [{ phase: "summary", lines: makeLines(discLineCount, discStart, "B", "disc") }],
          },
          {
            type: "outro",
  ...
  it("任意の有効な enriched script でパースは Ok を返す", () => {
    fc.assert(
      fc.property(validEnrichedScriptArb, (script) => {
        const result = parseEnrichedScript(JSON.stringify(script), "/p/a.wav");
        expect(result.isOk()).toBe(true);
      }),
      { numRuns: 30 },
    );
  ...
  it("不変条件: 全ライン.audioPath === wavPath", () => {
    fc.assert(
      fc.property(validEnrichedScriptArb, fc.string({ minLength: 1 }), (script, wavPath) => {
        const result = parseEnrichedScript(JSON.stringify(script), wavPath);
        expect(result.isOk()).toBe(true);
        if (result.isOk()) {
          result.value.lines.forEach((line) => {
  ...
  it("不変条件: 全 SectionMarker で startSec <= endSec", () => {
    fc.assert(
      fc.property(validEnrichedScriptArb, (script) => {
        ...
        expect(result.isOk()).toBe(true);
        if (result.isOk()) {
          result.value.sectionMarkers.forEach((marker) => {
      ...
      fc.property(validEnrichedScriptArb, (script) => {
        const totalInputLines = script.sections.reduce((acc, section) => {
          if (section.type === "intro") {
            ...
            return acc + section.blocks.reduce((sum, block) => sum + block.lines.length, 0);
          } else {
        ...
        expect(result.isOk()).toBe(true);
        if (result.isOk()) {
          expect(result.value.lines).toHaveLength(totalInputLines);
        }
      }),
      { numRuns: 30 },
    );
