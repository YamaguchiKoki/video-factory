...
describe("parseEnrichedScript — 異常系: 空の配列", () => {
  it("discussion block の lines が空のとき SCHEMA_VALIDATION_ERROR を返す", () => {
    ...
    const result = parseEnrichedScript(
      JSON.stringify(emptyDiscLines),
      WAV_PATH,
    );
    expect(result.isErr()).toBe(true);
    if (result.isErr()) {
...
describe("parseEnrichedScript — 正常系: newsItems", () => {
  ...
  it("複数の newsItems が全て引き継がれる", () => {
    ...
    if (result.isOk()) {
      ...
      expect(result.value.newsItems.map((n) => n.id)).toEqual([
        "news-1",
        "news-2",
        "news-3",
      ]);
...
describe("parseEnrichedScript — 正常系: SectionMarkers", () => {
  ...
  describe("IntroSectionMarker", () => {
    it("type が 'intro' である", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const intro = result.value.sectionMarkers.find(
          (m) => m.type === "intro",
        );
  ...
  describe("DiscussionSectionMarker（各 block が独立した marker になる）", () => {
    it("discussion の各 block に対して DiscussionSectionMarker が生成される", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const discussions = result.value.sectionMarkers.filter(
          (m) => m.type === "discussion",
        );
  ...
  describe("OutroSectionMarker", () => {
    it("type が 'outro' である", () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const outro = result.value.sectionMarkers.find(
          (m) => m.type === "outro",
        );
